...
const swiperGallerys = document.querySelector('.gallery-slider');
const swiperMarkets = document.querySelector('.market-slider');
...
if (document.querySelector('.market-slider')) {
  const swiperMarket = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](swiperMarkets, {
    slidesPerView: 1,
    // loop: true,
    spaceBetween: 30,
    navigation: {
      nextEl: '.market-slider__next',
      prevEl: '.market-slider__prev'
    },
    breakpoints: {
      651: {
        slidesPerView: 2
...
class GraphLazyVideo {
  constructor(container) {
    ...
    this.videoUrl = container.dataset.video;
    this.isFile = container.dataset.file === "true";
    this.thumbnail = container.querySelector('.video__thumbnail');
  ...
  check() {
    if (!this.videoUrl) {
      console.error("Ошибка: не указан data-video у блока", this.container);
    }
    if (!this.playButton) {
      console.error("Ошибка: не найдена кнопка воспроизведения");
    }
